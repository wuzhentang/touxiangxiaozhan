...
__all__ = ["CHARSET", "DATABASE_NAME", "TABLE_NAME", \
        "get_user_info", "add_new_user", "update_user_image"]
...
def connect_db():
    ...
    try:
        ...
        sys.stderr.write("Unable to connect to database %s: %d, %s" 
		    % (DATABASE_NAME, ex.args[0], ex.args[1]))
    else:
        return conn


def get_user_info(login_type, login_name):
    """Fetch from database,return a dict about information"""
    if login_type == "用户名":
        login_type = "username"
    ...
    if count > 0:
        user = cur.fetchone()
        user_info["uid"] = user[0]
        user_info["username"] = user[1]
        user_info["password"] = user[2]
        user_info["email"] = user[3]
        user_info["picture_name"] = user[4]
        user_info["register_time"] = user[5]
    cur.close()
    conn.close()
    return user_info
...
def add_new_user(username, password, email, pic_file_name):
    ...
    insert_sql = "INSERT INTO " + TABLE_NAME + \
	            "(username, password, email, picture_name, register_time) \
                VALUES(%s, %s, %s, %s, %s)"
    ...
    cur.execute(insert_sql, 
	            (username, password, email, pic_file_name, register_time))
    conn.commit()
    cur.close()
    conn.close()
	
def update_user_image(uid, new_file_name):
    """update user avator file name."""
    conn = connect_db()
    conn.select_db(DATABASE_NAME)
    cur = conn.cursor()
    update_sql = "UPDATE " + TABLE_NAME + " SET picture_name=" + '"' + \
	                new_file_name + '"' + " WHERE UID = " + '"' + uid + '"'
